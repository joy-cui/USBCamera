...
public  abstract class AbstractUVCCameraHandler extends Handler {
	...
	public interface OnPreViewResultListener{
		void onPreviewResult(byte[] data,int width,int height);
		void onPreviewResult(ByteBuffer data,int width,int height);
		void onPreviewResult(ByteBuffer y,ByteBuffer u,ByteBuffer v,int width,int height);
...
//			thread.handleStartRecording((String)msg.obj);
	...
	public  static final class CameraThread extends Thread {
		...
		private  WeakReference<CameraViewInterface> mWeakCameraView = null;
...
//		}
		...
		public void handleStartPreview(final Object surface) {
			...
			try {
				mUVCCamera.setPreviewSize(mWidth, mHeight, 1, 25, mPreviewMode, mBandwidthFactor);
				// 获取USB Camera预览数据，使用NV21颜色会失真
				// 无论使用YUV还是MPEG，setFrameCallback的设置效果一致
				mUVCCamera.setFrameCallback(mIFrameCallback, UVCCamera.PIXEL_FORMAT_YUV420SP);
//				mUVCCamera.setFrameCallback(mIFrameCallback, -1);
			} catch (final IllegalArgumentException e) {
				try {
					// fallback to YUV mode
					mUVCCamera.setPreviewSize(mWidth, mHeight, 1, 25, UVCCamera.DEFAULT_PREVIEW_MODE, mBandwidthFactor);
...
//			isAudioThreadStart = false;
		...
		private BlockingQueue<Object> mFrameList = new ArrayBlockingQueue<Object>(mfps);
...
//			Log.e("","onPreviewCode...start");
			...
			if (null != obj) {
				...
				ByteBuffer bufferY = ByteBuffer.wrap(data, 0, total);         // I420的Y分量
				ByteBuffer bufferU=ByteBuffer.allocate( total / 4);
				ByteBuffer bufferV=ByteBuffer.allocate( total / 4);
				for (int i = total; i < data.length; i += 2) {
					bufferV.put(data[i]);
					bufferU.put(data[i + 1]);
				}
				if (mPreviewListener != null) {
					mPreviewListener.onPreviewResult(bufferY, bufferU, bufferV, mWidth, mHeight);
				}
			}
//			Log.e("","onPreviewCode...end");
		...
		private final IFrameCallback mIFrameCallback = new IFrameCallback() {
			...
			public void onFrame(final ByteBuffer frame) {
				...
				final int  len = frame.capacity();
				Log.e("IFrameCallback","mIFrameCallback...callBack...回调 :"+len);
//				final int ySize=mWidth*mHeight;
////
//				final byte[] yuv = new byte[len];
//				frame.get(yuv);
//				if(mPreviewListener!=null){
////					mPreviewListener.onPreviewResult(frame,mWidth,mHeight);
//					mPreviewListener.onPreviewResult(yuv,mWidth,mHeight);
//				}
//				isCodec=true;
//				putVideoFrame(yuv,mWidth,mHeight);
//				if(mFrameThread==null){
